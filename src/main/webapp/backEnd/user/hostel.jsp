			...
			<section class="content">
	        	...
	        	<div class='row'>
		        	...
		        	<div class='col-md-5 col-sm-12 col-xs-12'>
		        		<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/hostel.do"
									id="hostelS">
							
							<b>民宿名稱:</b> 
							<input type="text" name="hostelName" placeholder="民宿名稱"><br>
							<br>							 
							<p>
							<b>民宿審核:</b> 
							<input type="checkbox" name="hostelVerification" value="1" class="flat-red"> 通過
							<input type="checkbox" name="hostelVerification" value="0" class="flat-red"> 未通過
							<br>
							<b>上下架:</b> 
							<input type="checkbox" name="hostelState" value="1" class="flat-red"> 上架
							<input type="checkbox" name="hostelState" value="0" class="flat-red"> 未上架
							<br>
							<input type="hidden" name="action" value="getlistByCQbk">			
							<p>				
							<p>
	        	...
	        	<div class='row'>
	        		<div class="col-xs-12">
			        		...
			        		<c:forEach var="hostelVO" items="${hostellist}" >
								<tr align='center' valign='middle'>
									<td>${hostelVO.hostelNo}</td>
									<td>${dealerSvc.getOneDealer(hostelVO.dealerNo).dealerName }</td>
									<td>${hostelVO.hostelName}</td>
									<td>${hostelVO.hostelAddress}</td>
									<td>${hostelVO.hostelVerification}</td>
									<td>
										<c:choose>
											<c:when test="${hostelVO.hostelState == '0'}">未上架</c:when>
											<c:otherwise>上架中</c:otherwise>
										</c:choose>								
									</td>
									<td>
										<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/backEnd/user/dealerProfile.jsp">
											    <button type="submit" class="btn btn-info btn-xs">顯示詳情 </button>
											    <input type="hidden" name="dealerNo" value="${hostelVO.dealerNo}">
