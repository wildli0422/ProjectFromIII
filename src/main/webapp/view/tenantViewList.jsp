...
<jsp:useBean id="viewlistSvc" scope="page"
	class="com.viewlist.model.ViewListService" />
<jsp:useBean id="viewinfoSvc" scope="page"
	class="com.viewinfo.model.ViewInfoService" />
<jsp:useBean id="viewphotoSvc" scope="page"
	class="com.viewphoto.model.ViewPhotoService" />

<%
	// Integer tenantNo = new Integer( request.getParameter("tenantNo"));
// pageContext.setAttribute("tenantNo", tenantNo);

// //Integer tenantNo = (Integer) request.getAttribute("tenantNo");
// ViewListService viewlistSvc1 = new ViewListService();
// Set<ViewListVO> viewlistlist = viewlistSvc1
...
// {
// 	System.out.println(viewlistVO.getViewlistno());
// }
...
ViewListService viewListService = new ViewListService();
Set<ViewListVO> viewlistlist = viewListService.getViewListByTenantno(tenantNo);
pageContext.setAttribute("viewlistVO", viewlistlist);
	...
	List<ViewInfoVO> viewinfolist = viewInfoService.getAll();
	pageContext.setAttribute("viewinfoVO", viewinfolist);

	ViewPhotoService viewPhotoService = new ViewPhotoService();
	List<ViewPhotoVO> viewphotolist = viewPhotoService.getOneViewpic();
	pageContext.setAttribute("viewphotoVO", viewphotolist);
	
	
	//System.out.println(tenantHasViewlist);
%>
...
<div class="col-md-12" id="topBar">
	...
	<div class="col-md-3" style="margin-top:20px;">
		<a href="<%=request.getContextPath()%>/HomePage.jsp.jsp"><img src="<%=request.getContextPath()%>/images/logo.png" id="logo"></a>
	</div>
	...
	<div id="content" class="col-md-10">
		...
		<div id="searchBar" class="row">
			...
			<div id="viewInfo" class="row">
				<div id="viewLabel">景點收藏</div>
				<!---viewLabel end--->


				<%
					if (tenantHasViewlist == 1) {
				%>

				<c:forEach var="viewlistVO" items="${viewlistVO}">

					<div id="viewDes">
						<div id="viewCard">
							<c:forEach var="viewphotoVO" items="${viewphotoSvc.oneViewpic}">
								<c:if test="${viewlistVO.viewno==viewphotoVO.viewno}">
									<a
										href="<%=request.getContextPath()%>/view/eachView.jsp?viewno=${viewphotoVO.viewno}"
							...
							<div id="viewContent">
								<c:forEach var="viewinfoVO" items="${viewinfoSvc.all}">
									<c:if test="${viewlistVO.viewno==viewinfoVO.viewno}">
										<p>${viewinfoVO.viewname}</p>
									</c:if>
								</c:forEach>
								...
								<FORM METHOD="post"
									...
									<input type="hidden" name="viewlistno"
										value="${viewlistVO.viewlistno}"> <input type="hidden"
										name="requestURL" value="<%=request.getServletPath()%>">
									<!--送出本網頁的路徑給Controller-->
									<input type="hidden" name="action" value="tenantViewListDelete">
									<input type="hidden" name="tenantNo"
										value="${viewlistVO.tenantNo}">
								</FORM>
