...
// 	   }
		function getRoomState(){
			$('#forAjax').load('<%=request.getContextPath()%>/room/emptyRoomType.jsp #forAjax #inner div');
...
<div class="col-md-12">
	...
	<div class="col-md-8"  style="margin-top:100px">
		...
		<div class="row">
			...
			<div class="col-md-1">			
			<div style="background-color:#23527c;height:30px;width:30px;"></div><p>訂房</p>
			</div>
			<div class="col-md-1">			
			<div style="background-color:#ff9999;height:30px;width:30px;"></div><p>已住房</p>
			</div>
			<div class="col-md-1">
			<div style="background-color:#99ff99;height:30px;width:30px;"></div><p>空房</p>
			</div>
			<div class="col-md-1">
			<div style="background-color:#ffcc99;height:30px;width:30px;"></div><p>清潔中</p>
			</div>
		</div>
		</div>
			...
			<div class="col-md-8">
				...
				現在時間:<span id="currTime" style="color: blue;"></span>
					...
					<c:forEach var="roomVO" items="<%=roomList%>">
						<div class="webRoomState" >
							
							<div class="col-md-3" style="text-align: center;" id="inner">
								<div style="font-size: 40px;">${roomVO.roomNo }
									<div style="font-size: 40px;">${roomTypeService.getOneRoomType(roomVO.roomTypeNo).roomTypeName }</div>
								</div>
...
// 				var messageArea =document.getElementById("webRoomState");
				...
				var message=jsonObj.roomNo+": "+jsonObj.roomState+","+jsonObj.hostelOrderDetailNo;
// 				alert("message: "+event.data);
// 				var room="#webRoomState"+jsonObj.roomNo;
// 				var roomState="#roomState"+jsonObj.roomNo;
				swal({title:"成功!",text: "房間修改完成",type: "success",showConfirmButton: false});
				setTimeout(function(){
					location.reload();
				},1500);
			...
			webSocket.onclose=function(event){
				updateStatus("web socket disconnect");
			}
