	...
	<script>
		...
		function drawCal(increment){
			...
			<c:forEach var="Date" items="${dateList}">
				$('input[id="date${Date}"]').attr("src","<%=request.getContextPath()%>/images/diamond_blue.png");
			</c:forEach>
		}
		...
		function calendar(year,month){
			...
			for(i=1;i<32;){
				date=new Date(year,month,i);
					...
					+'<input type="image" src="<%=request.getContextPath()%>/images/diamond_black.png" style="width:50px;height:50px;" id="date'+ year+"-"+(month+1)+"-"+i +'">'
					+'<input type="hidden" name="hostelNo" value="<%=hostelNo%>" >'
